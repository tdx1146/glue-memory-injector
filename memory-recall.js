...
// （127.0.0.1:18191），直调零 glue 改动（任务书授权：首选直调）；只读 fail-open。
// [2026-08-30 修复] V1 LMS :8190 已停用（四妹 V2 切换，agentos-v2/lms-api 监听 :18191）。
// 旧端口死等 LANDSCAPE_TIMEOUT_MS×N 会阻塞网关事件循环（实测 event_loop_delay 2s、
// models.list 4.7s、子代理启动 19.6s → UI 模型栏空、子AI「已停止」）。env 可覆盖。
const LMS_DEFAULT_URL = process.env.LMS_URL || "http://127.0.0.1:18191";
