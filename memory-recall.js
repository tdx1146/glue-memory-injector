...
//   1[回魂]状态块(保留) 2景观叙事(解读段扩权,非纯数字)
//   3thought notes(阶段 2 思考链接线:读 thoughts.jsonl 按激活度取 1-2 条)
//   4焦点记忆 3-5 条(Cowan 4±1,滤伪相关,带来源+置信度标注) 5质疑层 6行动层
...
// 阶段 2 思考链(2026-08-13):回魂段预算 200 → 300--3 thought notes 层
...
export { parseConfidenceTag, buildDoubtLayer, buildDiffuseProbe };
 ...
 */
export function buildDiffuseProbe(react, st, entropyRatio) {
...
/**
 * 5 质疑层(阶段 1 六层注入 + 阶段 3 数据源切换,设计 v1.1 §三-5)--"当前在怀疑什么"。
 *
...
// ("生成要求:1234"),与质疑层(描述"在怀疑什么")语义分离--质疑层=状态描述,
...
export function buildModulationConstraint(doubt) {
  ...
  if (tier.action === "strong") {
    return `[生成约束] 生成要求:1区分事实与推断 2不确定处明确标注"低置信" 3避免绝对化断言(一定/绝对/毫无疑问/必然)4关键结论给出替代候选(校准水位${wl})`;
  }
  return `[生成约束] 生成要求:1输出需校准,避免过度自信 2不确定处标注"低置信"(校准水位${wl})`;
}
...
// ── 3 thought notes(阶段 2 思考链,2026-08-13)──────────────────────────
 ...
 * 阶段 1 六层:1 状态块 + 2 景观叙事 + 3 thought 占位 + 自述/最近(保留)。
...
export function buildSoulText(data, maxChars = SOUL_MAX_CHARS, reactData = null, query = "", cfg = null, pickedThoughts = null) {
  ...
  // 2 景观叙事(解读段扩权,设计 v1.1 §三-2):描述"当前记忆状态"--
  ...
  // 3 thought notes(阶段 2 思考链,2026-08-13):按"与当前对话的激活度"
...
/**
 * 把 /recall 响应整理为注入文本(≤maxChars)--阶段 1 六层注入的 456 层:
 ...
 *   默认 null)。它的 action 字段(行动意向四问)经 buildActionLayer 注入6行动层;
...
export function buildContextText(data, query, maxChars, skipSelfRef = false, reactData = null, activatedThought = null) {
  ...
  // 4 焦点记忆:3-5 条(Cowan 4±1),精确去重(滤伪相关:同文不重复注入)
  const items = [];
  const seenTexts = new Set();
  for (const it of results) {
  ...
  // 5 质疑层 + 6 行动层(阶段 4:激活的 thought 才带行动意向;无则占位)
  ...
  if (joined.length > maxChars) {
    ...
    // (lost-in-the-middle:关键结构不埋在截断区;56 层不能先截)。
...
export async function buildMemoryContext(prompt, pluginConfig) {
  ...
  try {
    ...
    // 阶段 4:激活 thought 一次挑选、两处复用(3 thought 层 + 6 行动层)--
    ...
    // 阶段 2:query + cfg 透传(3 thought notes 激活度筛选需要)
    ...
    //   1 总量按 maxChars-COMPOSE_MARGIN=760 执行(40 字安全余量);
    //   2 thought ≤60 字/条 + 去重 + 单条上限(回魂段内「最近」不再被挤掉);
    //   3 回魂段保护优先级不变:[回魂] 永不先截(composeContext 先截记忆块)。
